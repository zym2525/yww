...
const resolveTypeName = (typeName: string) => {
  ...
  const typeLastName = typeName; // typeName.split('/').pop().split('.').pop();
  ...
  public getFuncationName(data: APIDataType) {
    ...
    return this.config.hook && this.config.hook.customFunctionName
      ...
      : // : data.method + this.genDefaultFunctionName(data.path, pathBasePrefix);
        this.genDefaultFunctionName(data.path, pathBasePrefix);
  }
    ...
    const data =
      ...
      [components.schemas].map((defines) => {
        ...
        return Object.keys(defines).map((typeName) => {
          ...
          const formatTypeName = this.config.formatTypeName
            ? this.config.formatTypeName(typeName)
            : typeName;
          return {
            typeName: resolveTypeName(formatTypeName),
            type: getDefinesType(),
